...
for i in range(len(data['ID'])):
    ...
    id = int(data['ID'][i])
    if(record_index[id] == 0 and day>=1 and month == 5):#只统计5.17号开始的数据
        matchResult = re.findall(keywords_wholeSymptoms, str(data['text'][i]), re.IGNORECASE)#匹配第一类
        # matchResult_2 = re.findall(keywords_second, str(data['text'][i]), re.IGNORECASE)  # 匹配第二类
        ...
        if ( matchResult != None and matchResult !=[]):  # 存在1类关键信息
            try:
                ...
                cv.circle(src, (position_x, position_y), 2, (86, 58, 180), 1)  # 画点
...
cv.imwrite(r"H:\wholeLook.png", src)
cv.waitKey(0)
